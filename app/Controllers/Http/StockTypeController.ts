...
import StockTypeRepository from 'App/Repositories/StockTypeRepository'
import { validationError } from 'App/Services/ResponseUtils'
import { StockTypeSchema } from 'App/Validators/StockTypeSchema'

export default class StockTypeController {
  constructor (private readonly repository = StockTypeRepository) {}
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: StockTypeSchema})
    } catch (error) {
      return response
        .status(422)
