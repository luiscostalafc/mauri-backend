...
import { UserUpdateSchema } from 'App/Validators'
import { MercadoPagoSchema } from 'App/Validators/MercadoPagoSchema'
...
export default class MercadoPagoController {
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: MercadoPagoSchema})
    } catch (error) {
      return response
        .status(422)
